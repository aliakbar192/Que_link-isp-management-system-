...
class CustomerForm extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      if (responsePackages.data.packages) {
        this.setState({
          packages: responsePackages.data.packages.map((p) =>
            _.pick(p, ["id", "packageName", "tenantId"])
          ),
        });
      }
    } catch (ex) {
    ...
    if (customerId && !isNaN(customerId)) {
      try {
        const { data: response } = await customerService.getCustomer(
          customerId
        );
        console.log(response);
        if (response.data.customer) {
          let customer = _.pick(response.data.customer, [
            "id",
            "name",
            "nicNo",
            "email",
      ...
      if (this.userRole.role === "tenant") {
        ...
        this.setState({
          data: {
            ...
            tenantId: this.userRole.tenantId + "",
            franchiseId: this.userRole.franchiseId + "",
            dealerId: this.userRole.dealerId + "",
          },
        });
      }
    }
  ...
  handlePackagesList = (tenantId) => {
    console.log("Tenant: " + tenantId);
    let availablePackages = this.state.packages.filter(
    ...
    if (
      availablePackages.filter((p) => p.tenantId == this.state.data.tenantId)
        .length === 0
    ) {
      const data = { ...this.state.data };
      data.tenantId = tenantId;
                    ...
                    <Col xs="8">
                      <h3 className="mb-0">
                        {this.state.data.id === 0
                          ? "Add Customer"
                          : `Update Customer: ${this.state.data.id}`}
                      </h3>
                    </Col>
                          ...
                          <Col md="4">
                            <FormGroup>
                              ...
                              >
                                ...
                                <option key="Expire" value="Expire">
                                  Expire
                                </option>
                                <option key="AutoRenew" value="AutoRenew">
                                  Auto Renew
                                </option>
                              </Input>
                              {this.state.errors.onExpire && (
                                <small className="text-danger mx-1">
                                  Select a valid "On Expire"
                                </small>
                              )}
                            </FormGroup>
                          </Col>
                          <Col md="4">
                            <FormGroup>
                              ...
                              >
                                ...
                                <option key="Ethernet" value="Ethernet">
                                  Ethernet
                                </option>
                              ...
                              {this.state.errors.connectionType && (
                                <small className="text-danger mx-1">
