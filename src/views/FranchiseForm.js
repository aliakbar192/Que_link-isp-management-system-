...
class FranchiseForm extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      const { data: responseNetworkServer } =
        await networkServerService.getNetworkServers();
      console.log(responseNetworkServer);
      if (responseNetworkServer.data.networkServers) {
        this.setState({
          networkServers: responseNetworkServer.data.networkServers.map((t) =>
            _.pick(t, ["id", "name", "tenantId"])
          ),
        });
      }
    } catch (ex) {
    ...
    if (franchiseId && !isNaN(franchiseId)) {
      try {
        ...
        if (response.data.franchise) {
          let franchise = _.pick(response.data.franchise, [
            "id",
            "name",
            "networkServerId",
            "tenantId",
  ...
  handleNetworkServerList = (tenantId) => {
    ...
    let availableNetworkServers = this.state.networkServers.filter(
      (ns) => ns.tenantId == tenantId
    );
    console.log(availableNetworkServers);
    this.setState({ availableNetworkServers });

    if (
      availableNetworkServers.filter(
        (ns) => ns.tenantId == this.state.data.tenantId
      ).length === 0
    ) {
      const data = { ...this.state.data };
                    ...
                    <Col xs="8">
                      <h3 className="mb-0">
                        {this.state.data.id === 0
                          ? "Add Franchise"
                          : `Update Franchise: ${this.state.data.id}`}
                      </h3>
                    </Col>
                          ...
                          <Col md="12">
                            <FormGroup>
                              ...
                              >
                                ...
                                {this.state.availableNetworkServers.map(
                                  (option) => (
                                    <option key={option.id} value={option.id}>
                                      {option.name}
                                    </option>
                              ...
                              {this.state.errors.networkServerId && (
                                <small className="text-danger mx-1">
                                  Select a valid "NAS"
                                </small>
                              )}
